 ...
 */
export function MonthlyPebbleSection({ report }: MonthlyPebbleSectionProps) {
  const {
    reportYear,
    reportMonth,
    monthlyPebbleCount,
  ...
  return (
    <div className="grid h-full grid-cols-[318px_350px_204px] items-center gap-x-[20px]">
      ...
      <section className="flex h-[262px] flex-col justify-center">
        ...
        <p className="text-[14px] font-medium leading-[150%] tracking-[-0.14px] text-[#737373]">
        ...
        <p className="mt-[14px] flex items-end gap-[8px]">
          ...
          <span className="bg-[linear-gradient(128deg,#D4D4D4_13.45%,#A3A3A3_37.1%,#737373_63.3%,#D4D4D4_89.14%)] bg-clip-text text-[88px] font-bold leading-none tracking-[-0.88px] text-transparent">
            {monthlyPebbleCount.toLocaleString('ko-KR')}
          </span>
          <span className="pb-[8px] text-[24px] font-bold leading-[130%] tracking-[-0.24px] text-[#404040]">
            개의 조약돌
          </span>
        </p>
      </section>
      ...
      {/* ---------- 중: 최근 3개월 차트 ---------- */}
      <section className="flex h-[262px] flex-col rounded-[20px] bg-white p-[20px]">
        <h3 className="text-[14px] font-medium leading-[150%] tracking-[-0.14px] text-[#737373]">
          최근 3개월
        </h3>
        <div className="mt-[8px] flex flex-1 flex-col">
          {/* 서버 값(recentMonths) */}
          <RecentMonthsChart months={recentMonths} />
        </div>
      </section>

      {/* ---------- 우: 누적 ---------- */}
      <section className="relative flex h-[262px] flex-col overflow-hidden rounded-[20px] bg-white p-[20px]">
        <h3 className="text-[14px] font-medium leading-[150%] tracking-[-0.14px] text-[#737373]">
          그리고 지금까지
        </h3>

        {/* 누적 개수 — 서버 값(totalPebbleCount) */}
        <p className="mt-[5px] bg-[linear-gradient(138deg,#D4D4D4_13.45%,#A3A3A3_37.1%,#737373_63.3%,#D4D4D4_89.14%)] bg-clip-text text-[48px] font-bold leading-none tracking-[-0.48px] text-transparent">
          {totalPebbleCount.toLocaleString('ko-KR')}
        </p>
        <p className="mt-[4px] text-[16px] font-bold leading-[130%] tracking-[-0.16px] text-[#404040]">
          개의 조약돌을 쌓았어요
        </p>

        {/* 기록 기간 — 서버 값(recordStartDate ~ recordEndDate) */}
        <p className="mt-[14px] text-[13px] font-normal leading-[130%] text-[#A3A3A3]">
