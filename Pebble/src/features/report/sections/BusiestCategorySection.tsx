...
/** 상단 마일스톤 타임라인 */
function MilestoneTimeline({ milestones }: { milestones: ReportMilestone[] }) {
            ...
            <span className="relative h-[12px] w-[12px]" aria-hidden="true">
              <img
                src={
                  milestone.featured
                    ? activeMilestoneIcon
                    : inactiveMilestoneIcon
                }
                alt=""
                className={
                  milestone.featured
                    ? 'absolute left-1/2 top-1/2 h-[20px] w-[20px] max-w-none -translate-x-1/2 -translate-y-1/2'
                    : 'h-[12px] w-[12px]'
                }
              />
            </span>
...
function MilestoneTaskColumn({
  ...
  return (
    <div className="relative flex h-[162px] min-w-0 flex-1 flex-col overflow-hidden rounded-[12px] bg-[rgba(255,255,255,0.05)] p-[20px]">
      <p className="flex items-center gap-[8px] text-[16px] font-medium leading-[150%] tracking-[-0.16px] text-[#171717]">
        <span
      ...
      <p className="absolute bottom-[11px] left-1/2 flex h-[30px] w-[130px] -translate-x-1/2 items-center justify-center rounded-[999px] bg-[#DAF4FF] text-center text-[13px] font-normal leading-[130%] text-[#A3A3A3]">
    ...
    <div className="flex h-full items-center gap-[20px]">
      ...
      <section className="w-[318px] shrink-0">
        ...
        <div className="mt-[32px] flex gap-[12px]">
          ...
          <div className="flex h-[134px] flex-1 flex-col justify-between rounded-[20px] bg-white p-[20px]">
            <span className="text-[54px] font-bold leading-none tracking-[-0.54px] text-[#404040]">
              {milestoneCount.toLocaleString('ko-KR')}
            </span>
            <span className="whitespace-nowrap text-[18px] font-medium leading-[150%] tracking-[-0.18px] text-[#A3A3A3]">
              등록한 마일스톤
            </span>
          </div>
          ...
          <div className="flex h-[134px] flex-1 flex-col justify-between rounded-[20px] bg-white p-[20px]">
            <span className="text-[54px] font-bold leading-none tracking-[-0.54px] text-[#737373]">
