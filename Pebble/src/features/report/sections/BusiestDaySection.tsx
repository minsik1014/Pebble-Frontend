...
/** 일정 한 줄 */
function ScheduleRow({ schedule }: { schedule: DaySchedule }) {
  // API 색상이 없는 목 데이터도 피그마의 상태색으로 안전하게 표시합니다.
  const barColor =
    schedule.colorHex ?? (schedule.completed ? '#FFECED' : '#DAF4FF');

  return (
    <li className="flex min-h-[60px] flex-col justify-center gap-[4px] overflow-hidden rounded-[20px] bg-white px-[20px] py-[12px]">
      {/* 경로 텍스트 — 서버 값(breadcrumb). 서버가 완성해서 보냅니다 */}
      {schedule.breadcrumb && (
        <p className="truncate text-[14px] font-medium leading-[150%] tracking-[-0.14px] text-[#A3A3A3]">
          {schedule.breadcrumb}
        </p>
      )}
      ...
      <div className="flex items-center justify-between gap-[12px]">
        <span className="flex min-w-0 items-center gap-[8px]">
          ...
          />
          {/* 일정 이름 — 서버 값(name) */}
          <span className="truncate text-[18px] font-medium leading-[150%] tracking-[-0.18px] text-[#404040]">
            {schedule.name}
          </span>
        </span>
  ...
  return (
    <div className="flex h-full w-full flex-col gap-[20px]">
      ...
      <div className="flex h-[40px] gap-[20px]">
        ...
        <span className="flex flex-1 items-center rounded-[999px] bg-[#FAFAFA] px-[20px] text-[16px] font-semibold leading-[150%] tracking-[-0.16px] text-[#A3A3A3]">
          {formatWeekday(date)}
        </span>
